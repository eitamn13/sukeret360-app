...
export function CommunityScreen({ onClose }: CommunityScreenProps) {
...
const handleSend = async () => {
  ...
  try {
    ...
    aiReply = data.reply || "לא הצלחתי לענות כרגע 😔";

  } catch (error) {
    aiReply = "שגיאה בחיבור לשרת 😢";
  }
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col animate-slide-in-right" style={{ background: theme.gradientFull }}>
          ...
          return (
            ...
            >
              <div className="flex items-start gap-3 mb-3">
                <div
                  className="w-10 h-10 rounded-xl flex items-center justify-center text-white text-sm font-800 flex-shrink-0"
                ...
                <div className="flex-1 text-right min-w-0">
                  ...
                  {tagStyle && post.tag && (
                    <span
                      className="inline-block mt-1 text-xs font-600 px-2 py-0.5 rounded-lg"
                      style={{ backgroundColor: tagStyle.bg, color: tagStyle.color, fontWeight: 600 }}
                    >
                      {post.tag}
                    </span>
                  )}
              ...
              >
                ...
                >
                  ...
                  <span className="text-xs font-600" style={{ fontWeight: 600 }}>{post.replies}</span>
                </button>
                <button
                ...
                >
                  <ThumbsUp size={15} strokeWidth={1.75} fill={isLiked ? theme.primary : 'none'} />
                  <span
                    className="text-xs font-600"
                    style={{ fontWeight: 600, color: isLiked ? theme.primary : '#9CA3AF' }}
                  >
                    {post.likes}
                  </span>
                </button>
      ...
      <div
        ...
        style={{ backgroundColor: 'white', borderTop: `1.5px solid ${theme.primaryBorder}`, boxShadow: `0 -4px 20px ${theme.primary}10` }}
      >
